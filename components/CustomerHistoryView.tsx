...
const InvoicesTable: React.FC<{invoices: Invoice[], onViewInvoice: (id: string) => void}> = ({ invoices, onViewInvoice }) => (
    <div>
        <h3 className="text-lg font-semibold text-gray-800 mb-2">Invoices (Treatments & Sales)</h3>
...
const PaymentsTable: React.FC<{payments: Payment[]}> = ({ payments }) => (
    <div>
        <h3 className="text-lg font-semibold text-gray-800 mb-2">Payments Received</h3>
        <div className="max-h-64 overflow-y-auto border rounded-lg">
            <table className="w-full min-w-max text-sm">
                <thead className="bg-gray-50 sticky top-0">
                    <tr className="border-b">
...
export const CustomerHistoryView: React.FC<CustomerHistoryViewProps> = ({ customer }) => {
  ...
  useEffect(() => {
    async function loadHistory() {
      setLoading(true);
      const data = await db.getCustomerHistory(customer.id, { startDate, endDate });
      setHistory(data);
      setLoading(false);
    }
  ...
  const renderContent = () => {
    if (activeTab === 'ledger') {
      return <CustomerLedgerView customer={customer} startDate={startDate} endDate={endDate} />;
    }
