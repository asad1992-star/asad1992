...
export const CustomerLedgerView: React.FC<CustomerLedgerViewProps> = ({ customer, startDate, endDate }) => {
  ...
  useEffect(() => {
    async function loadLedger() {
      setLoading(true);
      const data = await db.getCustomerLedger(customer, { startDate, endDate });
      setLedger(data);
      setLoading(false);
    }
    loadLedger();
        ...
        <table className="w-full min-w-max text-sm">
          <thead className="bg-gray-50 sticky top-0">
            <tr className="border-b">
              ...
              <th className="p-2 text-right font-semibold text-gray-600">Debit (Rs)</th>
              <th className="p-2 text-right font-semibold text-gray-600">Credit (Rs)</th>
              <th className="p-2 text-right font-semibold text-gray-600">Balance (Rs)</th>
            </tr>
          </thead>
